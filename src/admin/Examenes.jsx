      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <a href="/admin/dashboard" className="p-2 bg-white/10 hover:bg-white/20 rounded-xl transition-all">
          ...
          <button onClick={abrirFormularioNuevo} className="px-4 py-2 bg-violet-500 hover:bg-violet-400 text-white rounded-xl transition-all text-sm">
            + Nuevo Examen
          </button>
        </div>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900 border border-white/10 rounded-3xl p-8 shadow-2xl max-w-md w-full text-center">
              ...
              <div className="flex gap-3">
                <button onClick={() => setConfirmarEliminar(null)} className="flex-1 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-all font-medium">
                  Cancelar
                </button>
                <button onClick={() => eliminarExamen(confirmarEliminar.id)} className="flex-1 py-3 bg-red-500 hover:bg-red-400 text-white rounded-xl transition-all font-medium">
                  Sí, eliminar
                </button>
              </div>
            </div>
            ...
            <form onSubmit={guardarExamen} className="space-y-4">
              <div>
                ...
                <input type="text" value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Matemáticas - Unidad 1"
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-violet-300/40 focus:outline-none focus:ring-2 focus:ring-violet-400" required />
              </div>
              <div>
                ...
                <textarea value={descripcion} onChange={(e) => setDescripcion(e.target.value)} placeholder="Sumas y restas básicas"
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-violet-300/40 focus:outline-none focus:ring-2 focus:ring-violet-400" rows={2} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={duracion} onChange={(e) => setDuracion(parseInt(e.target.value))}
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-violet-400" min={5} max={180} required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-violet-100 mb-2">Fecha</label>
                  <input type="date" value={fecha} onChange={(e) => setFecha(e.target.value)}
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-violet-400" required />
                </div>
              </div>
              <div className="flex gap-3">
                <button type="button" onClick={cerrarFormulario} className="flex-1 py-3 bg-white/10 hover:bg-white/20 text-white font-medium rounded-xl transition-all">Cancelar</button>
                <button type="submit" className="flex-1 py-3 bg-violet-500 hover:bg-violet-400 text-white font-semibold rounded-xl transition-all">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1 ml-4">
                        ...
                        <button
                          ...
                          title={esFinalizado ? 'No se puede editar un examen finalizado' : 'Editar examen'}
                        >
                        ...
                        {/* Botón preguntas: solo si NO está finalizado */}
                        {esFinalizado ? (
                          <span
                            className="p-2 bg-white/5 text-white/20 rounded-lg transition-all text-sm cursor-not-allowed"
                            title="No se pueden gestionar preguntas de un examen finalizado"
                          >
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
