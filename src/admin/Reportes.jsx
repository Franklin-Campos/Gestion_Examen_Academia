...
export default function Reportes() {
  ...
  const cargarResultados = async (examenId) => {
    ...
    // Obtener resultados directamente con JOIN desde Supabase
    ...
    // También obtener alumnos que NO rindieron
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-white">Reportes de Notas</h1>
          </div>
        </div>

        {/* Filtro por examen */}
        <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-white/10 mb-6">
          <label className="block text-sm font-medium text-violet-100 mb-3">Filtrar por Examen</label>
          <select
        ...
        {examenSeleccionado && (
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-white/10">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={exportarCSV}
                className="px-4 py-2 bg-green-500/20 hover:bg-green-500/30 text-green-300 rounded-xl transition-all text-sm flex items-center gap-2"
              >
            ...
            {cargando ? (
              <div className="text-center py-8">
                ...
                <p className="text-white text-sm">Cargando resultados...</p>
              </div>
            ) : (
              <>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-white/10">
                        <th className="text-left py-3 px-2 text-violet-300 font-medium">DNI</th>
                        <th className="text-left py-3 px-2 text-violet-300 font-medium">Nombre</th>
                        <th className="text-center py-3 px-2 text-violet-300 font-medium">Nota</th>
                        <th className="text-center py-3 px-2 text-violet-300 font-medium">✅</th>
                        <th className="text-center py-3 px-2 text-violet-300 font-medium">❌</th>
                        <th className="text-center py-3 px-2 text-violet-300 font-medium">⚪</th>
                        <th className="text-center py-3 px-2 text-violet-300 font-medium">Estado</th>
                      </tr>
                    </thead>
                    <tbody>
                      {resultados.map((r, i) => (
                        <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                          <td className="py-3 px-2 text-white">{r.dni}</td>
                          ...
                          <td className="py-3 px-2 text-center text-green-300">{r.correctas}</td>
                          <td className="py-3 px-2 text-center text-red-300">{r.incorrectas}</td>
                          <td className="py-3 px-2 text-center text-gray-400">{r.sin_responder}</td>
                          <td className="py-3 px-2 text-center">
                      ...
                      <tr className="bg-white/5 font-medium">
                        <td className="py-3 px-2" colSpan={2}>
                          <span className="text-violet-300">PROMEDIO</span>
                        </td>
                        <td className="py-3 px-2 text-center text-white font-bold">{promedio()}</td>
                ...
                <div className="grid grid-cols-4 gap-4 mt-6 pt-6 border-t border-white/10">
                  <div className="text-center">
                    ...
                    <p className="text-violet-300/60 text-xs">🟢 Excelente</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-yellow-300">
                      {resultados.filter(r => r.color === 'ambar').length}
                    </p>
                    <p className="text-violet-300/60 text-xs">🟡 Puede mejorar</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-red-300">
                      {resultados.filter(r => r.color === 'rojo').length}
                    </p>
                    <p className="text-violet-300/60 text-xs">🔴 Desaprobado</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-gray-300">
                      {resultados.filter(r => r.estado === 'No se presentó').length}
                    </p>
                    <p className="text-violet-300/60 text-xs">⚪ No presentados</p>
                  </div>
                </div>
              </>
            )}
