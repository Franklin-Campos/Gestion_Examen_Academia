...
export default function Dashboard() {
  ...
  const regenerarPin = async (alumnoId, nuevoPin) => {
    ...
    // Actualizar PIN en la tabla alumnos
    const { error: updateError } = await supabase
      .from('alumnos')
      .update({ pin: nuevoPin })
      .eq('id', alumnoId)
  ...
  const eliminarAlumno = async (alumnoId) => {
    ...
    // Eliminar de auth.users (usando Edge Function o API)
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        {confirmarSalir && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900 border border-white/10 rounded-3xl p-8 shadow-2xl max-w-md w-full text-center">
              ...
              <p className="text-violet-300 text-sm mb-6">Serás redirigido al inicio de sesión del panel de administración.</p>
        ...
        {confirmarEliminarAlumno && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900 border border-white/10 rounded-3xl p-8 shadow-2xl max-w-md w-full text-center">
              ...
              <h3 className="text-xl font-bold text-white mb-2">¿Eliminar alumno?</h3>
              ...
              <div className="flex gap-3">
                <button onClick={() => setConfirmarEliminarAlumno(null)} className="flex-1 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-all font-medium">Cancelar</button>
                <button onClick={() => eliminarAlumno(confirmarEliminarAlumno.id)} className="flex-1 py-3 bg-red-500 hover:bg-red-400 text-white rounded-xl transition-all font-medium">Sí, eliminar</button>
              </div>
            </div>
          </div>
        )}
        ...
        {editandoAlumno && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900 border border-white/10 rounded-3xl p-8 shadow-2xl max-w-md w-full">
              <h2 className="text-xl font-bold text-white mb-6">Editar Alumno</h2>
              <form onSubmit={actualizarAlumno} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-violet-100 mb-2">DNI</label>
                  <input
                    type="text"
                    value={editandoAlumno.dni}
                    onChange={(e) => setEditandoAlumno({ ...editandoAlumno, dni: e.target.value })}
                ...
                <div>
                  <label className="block text-sm font-medium text-violet-100 mb-2">Nombre Completo</label>
                  <input
                    type="text"
                    value={editandoAlumno.nombre}
                    onChange={(e) => setEditandoAlumno({ ...editandoAlumno, nombre: e.target.value })}
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-violet-400"
                    required
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-violet-100 mb-2">PIN Actual: {editandoAlumno.pin}</label>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="w-full py-2 bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-300 rounded-xl transition-all text-sm"
                  >
                    Regenerar PIN
                  </button>
                </div>
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => setEditandoAlumno(null)} className="flex-1 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-all font-medium">Cancelar</button>
                  <button type="submit" disabled={cargando} className="flex-1 py-3 bg-violet-500 hover:bg-violet-400 disabled:bg-violet-600/50 text-white font-semibold rounded-xl transition-all">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-2">
            <a href="/admin/examenes" className="px-4 py-2 bg-violet-500/50 hover:bg-violet-500 text-white rounded-xl transition-all text-sm">Exámenes</a>
            <a href="/admin/reportes" className="px-4 py-2 bg-green-500/50 hover:bg-green-500 text-white rounded-xl transition-all text-sm">
              Reportes
            </a>
            <button onClick={() => setConfirmarSalir(true)} className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-all text-sm">Cerrar Sesión</button>
          </div>
        </div>

        {mensaje && (
          <div className="mb-6 rounded-2xl p-4 bg-green-600 text-white text-sm flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-white/10">
            ...
            <form onSubmit={registrarAlumno} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-violet-100 mb-2">DNI del Alumno</label>
                <input type="text" value={dni} onChange={(e) => setDni(e.target.value)} placeholder="12345678"
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-violet-300/40 focus:outline-none focus:ring-2 focus:ring-violet-400 focus:border-transparent transition-all" required maxLength={8} />
              </div>
              <div>
                <label className="block text-sm font-medium text-violet-100 mb-2">Nombre Completo</label>
                <input type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Juan Pérez"
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-violet-300/40 focus:outline-none focus:ring-2 focus:ring-violet-400 focus:border-transparent transition-all" required />
              </div>
          ...
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-white/10">
            <div className="flex items-center justify-between mb-6">
              ...
              {alumnos.length > 0 && (
                <span className="text-violet-300 text-sm">{alumnosFiltrados.length} de {alumnos.length}</span>
              )}
            </div>
            ...
            <div className="relative mb-4">
              ...
              {busqueda && (
                <button onClick={() => setBusqueda('')} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-white/10 rounded-lg transition-all">
            ...
            {alumnos.length === 0 ? (
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {alumnosFiltrados.map((alumno) => (
                  <div key={alumno.id} className="bg-white/5 rounded-xl p-4 border border-white/5 hover:border-white/10 transition-all">
